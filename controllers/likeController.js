...
export const likeBlog = async (request, response) => {
    ...
    try {
        ...
        if (!blog) {
            response.status(StatusCodes.BAD_REQUEST).send({ message: "Blog not found" });
        }
        const like = await Like.create({ user_ID: userId, blog_ID: blogId });
        response.status(StatusCodes.CREATED).send({ message: 'Blog liked successfully', like });
    } catch (error) {
...
export const unlikeBlog = async (request, response) => {
    try {
        ...
        response.status(StatusCodes.OK).send({ message: 'Blog unliked successfully', like });
    } catch (error) {
        response.status(StatusCodes.INTERNAL_SERVER_ERROR).send({ message: getReasonPhrase(StatusCodes.INTERNAL_SERVER_ERROR) });
    }
}
